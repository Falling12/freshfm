...
const Header: FC = () => {
  ...
  return (
    <header className={`text-white flex justify-between items-center p-4 z-2 fixed top-0 w-full transition-all ${navBlur ? 'backdrop-blur-xl bg-black/30' : ''}`}>
        <a href='#'>
            <img src='/logo.png' alt='logo' />
        </a>
        
        <div className={`flex md:flex-col items-center gap-5 text-[24px] md:hidden`}>
            <a className='cursor-pointer' href='#program'>Műsorlista</a>
            <a className='cursor-pointer'>Rólunk</a>
            <a className='cursor-pointer'>Csapatunk</a>
            <button role={'button'} className='supportbtn px-5 py-2 rounded-xl font-medium ml-3'>Támogatás</button>
        </div>

        <svg className='md:flex hidden cursor-pointer' onClick={() => setNavOpen(!navOpen)} width="33" height="33" viewBox="0 0 33 33" fill="none" xmlns="http://www.w3.org/2000/svg">
