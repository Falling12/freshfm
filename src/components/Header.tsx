...
const Header: FC = () => {
  ...
  return (
    <header className={`text-white flex justify-center items-center z-50 select-none fixed top-0 w-full transition-all duration-500 ${navOpen ? 'md:!flex-col md:w-full md:bg-black/30' : ''} ${navBlur ? 'backdrop-blur-xl bg-black/30' : ''}`}>
        <div className="flex items-center justify-between p-4 max-w-[1500px] w-full">
            <a href='#' className={`block md:w-[50px] ${navOpen ? 'md:!hidden' : ''}`}>
                <img src='/logo.png' alt='logo' />
            </a>

            <div className={`hidden ${navOpen ? 'md:!flex' : ''} justify-between items-center flex-1 w-full`}>
                <a href='#'>
            ...
            <div className={`flex md:flex-col items-center gap-5 text-[24px] md:hidden ${navOpen ? 'md:!flex' : ''}`}>
            ...
            <svg className={`md:block hidden cursor-pointer hover:opacity-75 transition-all ${navOpen ? 'md:!hidden' : ''}`} onClick={() => setNavOpen(!navOpen)} width="33" height="33" viewBox="0 0 33 33" fill="none" xmlns="http://www.w3.org/2000/svg">
